...
connect-image:
	docker container exec -it  sam-service /bin/bash
#curl 127.0.0.1:3000 > html.test
COMMAND=/bin/bash
run-image-bash:
	docker run -it --rm --name sam-service \
	--entrypoint ${COMMAND} \
	--env-file=.env \
...
run-image:
	docker run --rm -p 3000:3000 --name  sam-service --env-file=.env \
	-v ${PWD}/app:/app/app \
	-v ${PWD}/lib:/app/lib \
	-v ${PWD}/log:/app/log \
	-v ${PWD}/db:/app/db \
...
run-image-tests:
	...
	-v ${PWD}/db:/app/db \
	-e SAML_DB_HOST=${LOCAL_IP} \
	--name  sam-service  sam-service "rspec -fd ${FILE}"